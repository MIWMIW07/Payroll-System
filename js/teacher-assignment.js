...
const ASSIGNMENT_TYPES = {
    ...
    ADMIN_SHS: 'admin_shs',
    ADMIN_SHS_COLLEGE: 'admin_shs_college',
    GUARD: 'guard',
    SA: 'sa'
};
...
function getSHSTeachers() {
    return teacherAssignments.filter(t => 
        (
            t.assignment === ASSIGNMENT_TYPES.SHS_ONLY ||
            t.assignment === ASSIGNMENT_TYPES.BOTH ||
            t.assignment === ASSIGNMENT_TYPES.ADMIN_SHS ||
            t.assignment === ASSIGNMENT_TYPES.ADMIN_SHS_COLLEGE
        ) &&
        t.status === 'Active'
    );
}
...
function getCollegeTeachers() {
    return teacherAssignments.filter(t => 
        (
            t.assignment === ASSIGNMENT_TYPES.COLLEGE_ONLY ||
            t.assignment === ASSIGNMENT_TYPES.BOTH ||
            t.assignment === ASSIGNMENT_TYPES.ADMIN_COLLEGE ||
            t.assignment === ASSIGNMENT_TYPES.ADMIN_SHS_COLLEGE
        ) &&
        t.status === 'Active'
    );
...
function getAdminStaff() {
    return teacherAssignments.filter(t =>
        (
            t.assignment === ASSIGNMENT_TYPES.ADMIN ||
            t.assignment === ASSIGNMENT_TYPES.ADMIN_COLLEGE ||
            t.assignment === ASSIGNMENT_TYPES.ADMIN_SHS ||
            t.assignment === ASSIGNMENT_TYPES.ADMIN_SHS_COLLEGE
        ) &&
...
function teachesSHS(teacherId) {
    ...
    return teacher && (
        teacher.assignment === ASSIGNMENT_TYPES.SHS_ONLY ||
        teacher.assignment === ASSIGNMENT_TYPES.BOTH ||
        teacher.assignment === ASSIGNMENT_TYPES.ADMIN_SHS ||
        teacher.assignment === ASSIGNMENT_TYPES.ADMIN_SHS_COLLEGE
    );
}
...
function teachesCollege(teacherId) {
    const teacher = getTeacherById(teacherId);
    return teacher && (
        teacher.assignment === ASSIGNMENT_TYPES.COLLEGE_ONLY ||
        teacher.assignment === ASSIGNMENT_TYPES.BOTH ||
        teacher.assignment === ASSIGNMENT_TYPES.ADMIN_COLLEGE ||
        teacher.assignment === ASSIGNMENT_TYPES.ADMIN_SHS_COLLEGE
    );
}
        ...
        <div class="space-y-4">
            <div>
                ...
                <select id="teacherAssignment" class="w-full px-4 py-2 border border-gray-200 rounded-lg" onchange="toggleAssignmentFields()">
                    ...
                    <option value="admin_college" ${teacher?.assignment === 'admin_college' ? 'selected' : ''}>Admin and College</option>
                    <option value="admin_shs" ${teacher?.assignment === 'admin_shs' ? 'selected' : ''}>Admin and SHS</option>
                    <option value="admin_shs_college" ${teacher?.assignment === 'admin_shs_college' ? 'selected' : ''}>Admin, SHS, and College</option>
...
function toggleAssignmentFields() {
    ...
    const hasShs = assignment === 'shs_only' || assignment === 'both' || assignment === 'admin_shs' || assignment === 'admin_shs_college';
    const hasCollege = assignment === 'college_only' || assignment === 'both' || assignment === 'admin_college' || assignment === 'admin_shs_college';
    const hasAdmin = assignment === 'admin' || assignment === 'admin_college' || assignment === 'admin_shs' || assignment === 'admin_shs_college';

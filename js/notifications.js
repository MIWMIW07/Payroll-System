...
export async function markAsRead(id) { await markNotificationAsRead(id); await refreshNotifications(); }
window.markAllAsRead = async function() { const n = await getNotifications(false); for (const n2 of n) await markAsRead(n2.id); await refreshNotifications(); };
export function cleanupNotifications() { if (notificationInterval) clearInterval(notificationInterval); }
    ...
    pageEmployees.forEach(emp => {
        ...
        switch(emp.assignment) {
            ...
            case 'admin':
                ...
                break;
            case 'admin_college':
                assignmentBadge = '<span class="bg-emerald-100 text-emerald-600 text-xs px-2 py-1 rounded-full">Admin+CLG</span>';
                rateDisplay = `₱${emp.rate_admin || 70}/₱${emp.rate_college || 85}`;
                break;
            case 'admin_shs':
                assignmentBadge = '<span class="bg-teal-100 text-teal-600 text-xs px-2 py-1 rounded-full">Admin+SHS</span>';
                rateDisplay = `₱${emp.rate_admin || 70}/₱${emp.rate_shs || 80}`;
                break;
            case 'admin_shs_college':
                assignmentBadge = '<span class="bg-cyan-100 text-cyan-600 text-xs px-2 py-1 rounded-full">Admin+SHS+CLG</span>';
                rateDisplay = `₱${emp.rate_admin || 70}/₱${emp.rate_shs || 80}/₱${emp.rate_college || 85}`;
                break;
            case 'guard':
